...
save(DBName, Doc, Key) ->
	K16=integer_to_list(Key, 16),
	{St1,St2,St3}= erlang:timestamp(),
	io:fwrite("[BEGIN] save - key:~p ~p~p~p~n", [K16,St1,St2,St3]),
	Insert_return=insert(Doc, Key, DBName),
	{En1,En2,En3}= erlang:timestamp(),
	io:fwrite("[END] save - key:~p ~p~p~p~n", [K16,En1,En2,En3]),
	Insert_return.
...
save_doc(PosLastDoc, LastVersion, Doc, Settings) ->
	...
	file:close(Fp),
	%compress_index(Settings#dbsettings.dbname),
