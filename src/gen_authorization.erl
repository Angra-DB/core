...
behaviour_info(callbacks) ->
  [{init, 2}, {handle_request_permission, 4}, {handle_grant_permission, 3}, {handle_revoke_permission, 3}, {handle_show_permission, 3}].
...
request_permission(Authorization_scheme, Persistence_scheme, Database, Username, {Command, Args}) ->
  Authorization_scheme:handle_request_permission(Persistence_scheme, Database, Username, {Command, Args}).
...
grant_permission(Authorization_scheme, Persistence_scheme, Database, Grant_args) ->
  Authorization_scheme:handle_grant_permission(Persistence_scheme, Database, Grant_args).
...
revoke_permission(Authorization_scheme, Persistence_scheme, Database, Revoke_args) ->
  Authorization_scheme:handle_revoke_permission(Persistence_scheme, Database, Revoke_args).
...
show_permission(Authorization_scheme, Persistence_scheme, Database, Args) ->
  Authorization_scheme:handle_show_permission(Persistence_scheme, Database, Args).